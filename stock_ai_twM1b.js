...
let payload = getPayload("twA02", 2000, 1, 2017, 8);
let dataL = [];
	...
	if (!error && response.statusCode == 200) {
		...
		for(let i = 0; i < content.rows.length; i++) {
			let _row = {
				...
				price: content.rows[i].sPrice // M1b (十億)
			};
			//console.log(_row);	
			dataL.push(_row);
		}

    mgLib.updateTwM1b(dataL, {upsert:true}, function(err) {
      if(err) {
        console.log("update Taiwan M1b error !!");
      } else {
        console.log("update Taiwan M1b successfully !!");
      }
    });
	}
});